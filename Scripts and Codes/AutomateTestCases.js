...
async function automateTest() {
    const file = reader.readFile('./Mental_Illness_Input_and_Output.xlsx')

    let data = []
    let indexStarts = 0

    let testCaseName = "Sheet2"
    ...
    for(const row of data) {
        if(counter >= indexStarts) {
            ...
            let expectedTextOutput = row["Output"]
...
async function runTest (textInput, expectedTextOutput, similarityPassedThreshold) {
    ...
    const driver = await wdio.remote({
        ...
        let hasToClick4 = bagOfTexts.includes("Not right now")
        ...
        if(hasToClick4) {
            await driver.$("~Not right now").click();
            await driver.pause(5000);
        }
        ...
        fs.appendFile("./MentalIllness2.txt", 
        "Input: " + textInput + "\n"
